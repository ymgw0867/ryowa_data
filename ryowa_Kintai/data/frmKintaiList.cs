...
namespace ryowa_Kintai.data
{
    public partial class frmKintaiList : Form
    {
        ...
        kintaiDataSet dts = new kintaiDataSet();
        kintaiDataSetTableAdapters.T_勤怠TableAdapter adp = new kintaiDataSetTableAdapters.T_勤怠TableAdapter();
        kintaiDataSetTableAdapters.M_社員1TableAdapter sAdp = new kintaiDataSetTableAdapters.M_社員1TableAdapter();
        kintaiDataSetTableAdapters.M_社員TableAdapter aAdp = new kintaiDataSetTableAdapters.M_社員TableAdapter();
        kintaiDataSetTableAdapters.M_休日TableAdapter hAdp = new kintaiDataSetTableAdapters.M_休日TableAdapter();
        kintaiDataSetTableAdapters.M_工事TableAdapter kAdp = new kintaiDataSetTableAdapters.M_工事TableAdapter();
        ...
        string cEDate = "col25";
        string cCode = "col26";
        string cHayade = "col44";       // 2018/07/11 早出
        ...
        string cCode2 = "col43";
        string cHayade2 = "col45";       // 2018/07/11 早出
        #endregion  
        ...
        private void GridViewSetting(DataGridView g, int lgType)
        {
            try
            {
                ...
                else
                {
                    ...
                    foreach (DataGridViewColumn c in g.Columns)
                    {
                        if (c.Name == cJyosetsu || c.Name == cTokushu ||
                            c.Name == cTooshi || c.Name == cYakan || c.Name == cShokumu)
                        {
                            c.Visible = false;
                        }
            ...
            try
            {
                for (int iX = 0; iX < dt.Day; iX++)
                {
                    ...
                    if (dts.T_勤怠.Any(a => a.日付 == sDt && a.社員ID == sNum))
                    {
                        ...
                        {
                            g[cAccount, iX].Value = t.M_社員Row.氏名;
                        }

                        g[cDate, iX].Value = t.登録年月日;

                        if (t.M_社員1RowByM_社員1_T_勤怠1 == null)
                        {
                            g[cEAccount, iX].Value = string.Empty;
                        }
                        else
                        {
                            g[cEAccount, iX].Value = t.M_社員1RowByM_社員1_T_勤怠1.氏名;
                        }
        ...
        {
            // 出勤簿表示
            //showKintaiData();     // 2018/09/17 コメント化
        }
        ...
        private void showSumData(DataGridView g, int sYY, int sMM, int sNum, double fZan)
        {
            var ss = dts.T_勤怠
                ...
                {
                    pID = gr.Key,
                    pHayade = gr.Sum(a => a.早出残業),  // 2018/07/11
            ...
            //fZan *= 60;       // 固定残業時間を分単位に変換 2018/09/20 コメント化 2018/09/25

            fZan = 0;           // 出勤簿・車両走行報告書には固定残業時間を反映しない 2018/09/25
            ...
            foreach (var t in ss)
            {
                 ...
                 * 出勤簿・車両走行報告書には固定残業時間を反映しない(fZanは0) 2018/09/25
                 * ------------------------------------------------------------------
                 */
        ...
        private void dispInitial()
        {
            ...
            txtYear.Text = DateTime.Today.Year.ToString();      // 西暦に変更：2018/07/12
            ...
            if (e.ColumnIndex == 0)
            {
                if (DateTime.TryParse(sYY.ToString() + "/" + sMM.ToString() + "/" + (e.RowIndex + 1).ToString(), out dt))
                {
                    ...
                    else
                    {
                        ...
                        // 出勤簿データ再表示
                        //adp.FillByYYMM(dts.T_勤怠, Utility.StrtoDecimal(txtYear.Text), Utility.StrtoDecimal(txtMonth.Text));
            ...
            if (changeStatus)
            {
                    ...
                    cName == cTooshi || cName == cYakan || cName == cShokumu)
                {
                    ...
                    if (dg[cKakunin, r].Value.ToString() == global.chkBoxTrue)
                    {
                        ...
                        // 確認チェックを行った管理者の社員IDを書き込む 2016/04/13
                        s.確認印 = global.loginUserID;
                    }
                    else
                    {
                        s.確認印 = global.flgOff;
        ...
        private void sReport()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // 年月
                    //oxlsPrintSheet.Cells[2, 5] = Properties.Settings.Default.gengou;
