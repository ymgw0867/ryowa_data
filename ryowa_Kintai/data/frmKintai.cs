...
namespace ryowa_Kintai.data
{
    public partial class frmKintai : Form
    {
        ...
        int _chkManager = 0;    // 確認印チェック

        kintaiDataSet dts = new kintaiDataSet();
        kintaiDataSetTableAdapters.T_勤怠TableAdapter kAdp = new kintaiDataSetTableAdapters.T_勤怠TableAdapter();
        kintaiDataSetTableAdapters.M_社員TableAdapter sAdp = new kintaiDataSetTableAdapters.M_社員TableAdapter();
        kintaiDataSetTableAdapters.M_休日TableAdapter hAdp = new kintaiDataSetTableAdapters.M_休日TableAdapter();
        kintaiDataSetTableAdapters.M_工事TableAdapter bAdp = new kintaiDataSetTableAdapters.M_工事TableAdapter();
        ...
        private void dispInitial()
        {
            ...
            txtHH.Text = string.Empty;      // 2018/07/11 早出残業・時
            txtHM.Text = string.Empty;      // 2018/07/11 早出残業・分
        ...
        private bool errCheck()
        {
            ...
            if (rBtnWork.Checked || rBtnHolWork.Checked)
            {
                ...
                if (r > w)
                {
                    MessageBox.Show("残業時間が勤務時間（" + Properties.Settings.Default.restTime + "分休憩を除く）を超えています", "残業時間確認", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    txtZH.Focus();
                    return false;
                }
        ...
        private void chkManager_CheckedChanged(object sender, EventArgs e)
        {
            if (chkManager.Checked)
            {
                _chkManager = global.loginUserID;
            }
            else
            {
                _chkManager = global.flgOff;
            }
        }
    }
}
