...
namespace ryowa_Genba.common
{
    public partial class frmLogin : Form
    {
        ...
        private bool SerachloginUser()
        {
            ...
            // ユーザー認証
            if (!dts.M_社員.Any(a => a.ID.ToString() == txtName.Text && a.パスワード == txtPassword.Text))
            {
                lTry++;
            ...
            {
                var s = dts.M_社員.Single(a => a.ID.ToString() == txtName.Text && a.パスワード == txtPassword.Text);
        ...
        {
            // ファイルを削除
            if (System.IO.File.Exists(@"c:\ryowa_gdata\xls\" + ID_PASS))
            {
                System.IO.File.Delete(@"c:\ryowa_gdata\xls\" + ID_PASS);
            }

            // CSVファイル出力
            if (isPut)
            {
                System.IO.File.WriteAllText(@"c:\ryowa_gdata\xls\" + ID_PASS, txtName.Text + "," + txtPassword.Text, System.Text.Encoding.GetEncoding("utf-8"));

                // ID_PASSの属性を取得する
                System.IO.FileAttributes attr = System.IO.File.GetAttributes(@"c:\ryowa_gdata\xls\" + ID_PASS);

                // 隠しファイル属性を追加する
                System.IO.File.SetAttributes(@"c:\ryowa_gdata\xls\" + ID_PASS,
        ...
        private void frmLogin_Shown(object sender, EventArgs e)
        {
            ...
            if (System.IO.File.Exists(@"c:\ryowa_gdata\xls\" + ID_PASS))
            {
                foreach (var item in System.IO.File.ReadLines(@"c:\ryowa_gdata\xls\" + ID_PASS))
                {
                    string[] p = item.Split(',');
                    if (p.Length == 2)
                    {
